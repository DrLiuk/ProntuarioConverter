...
def calcola(tipo,larghezza,altezza,spessore):
    ...
    calcolaWindow.geometry('550x150')
    ...
    def converti():
        print(peso)
        if metriEntry.index('end') == 0:
            metriRes = prodotti.kgToMt(int(kiliEntry.get()),peso)
            metriEntry.delete(0,END)
            metriEntry.insert(0,metriRes)
        elif kiliEntry.index('end') == 0:
            kiliRes = prodotti.mtToKg(int(metriEntry.get()),peso)
    ...
    metriEntry = Entry(calcolaWindow,font=(font_One,18),width=15)
    ...
    kiliEntry = Entry(calcolaWindow,font=(font_One,18),width=15)
...
def create_main_window():
    ...
    mainWindow.geometry('700x520+0+0')
    ...
    larghezzaEntry = Entry(mainWindow_larghezzaFrame,textvariable=larghezza,background='#1E1E24',foreground='white',font=(font_One,14))
    ...
    altezzaEntry = Entry(mainWindow_altezzaFrame,textvariable=altezza,background='#1E1E24',foreground='white',font=(font_One,14))
    ...
    spessoreEntry = Entry(mainWindow_spessoreFrame,textvariable=spessore,background='#1E1E24',foreground='white',font=(font_One,14))
